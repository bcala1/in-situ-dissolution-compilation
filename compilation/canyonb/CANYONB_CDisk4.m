...
tic
% Load WOA variables
m = readmatrix('../data/CDisk4_forCANYONB_2067.csv', ...
    Delimiter=",");
...
headers = ["Latitude", "Longitude", "Depth", "Pressure", ...
    "InSitu_Temperature", "Salinity+0.01", "DissolvedOxygen", "Year", "Month", ...
    "Nitrate", "Phosphate", "Silicic acid", "Alkalinity", "DIC"];

save_mat = nan(length(m(:,1)), length(headers));
writematrix(headers, "../data/CDisk4_CANYONBresults_2067.csv");
...
writematrix(save_mat, "../data/CDisk4_CANYONBresults_2067.csv", 'writemode', 'append');
