...
tic
% Load WOA variables
m = readmatrix('../data/in_situ_rates_compiled_withWOA_forCANYONB.csv', ...
    Delimiter=",");
...
headers = ["Latitude", "Longitude", "Depth", "Pressure", ...
    "InSitu_Temperature+0.001", "Salinity", "Doxy", "Year", "Month", ...
    "NO3", "PO4", "SiOH4", "TA", "DIC", 'pH_insitu'];

save_mat = nan(length(m(:,1)), length(headers));
writematrix(headers, "../data/compiled_WOA_CANYONBresults_uncertainty_dT.csv");
...
writematrix(save_mat, "../data/compiled_WOA_CANYONBresults_uncertainty_dT.csv", 'writemode', 'append');
